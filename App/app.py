...
def cleanResume(txt):
    cleanText = re.sub(r'http\S+\s', ' ', txt)  # Raw string for URLs
    cleanText = re.sub(r'RT|cc', ' ', cleanText)  # Raw string for 'RT' or 'cc'
    cleanText = re.sub(r'#\S+\s', ' ', cleanText)  # Raw string for hashtags
    cleanText = re.sub(r'@\S+', '  ', cleanText)  # Raw string for mentions
    cleanText = re.sub(r'[%s]' % re.escape(r"""!"#$%&'()*+,-./:;<=>?@[\]^_`{|}~"""), ' ', cleanText)  # Raw string for special characters
    cleanText = re.sub(r'[^\x00-\x7f]', ' ', cleanText)  # Raw string for non-ASCII characters
    cleanText = re.sub(r'\s+', ' ', cleanText)  # Raw string for whitespace
    return cleanText
